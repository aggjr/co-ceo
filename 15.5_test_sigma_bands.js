...
/**
 * APOLLO ENGINE v15.5 - SIGMA BANDS EXPERIMENT
 * Replaces fixed percentages with probabilistic Sigma offsets.
 * Centro = Ideal = ACIMA.
 */
...
const Z_SCORE_TARGET = 2.33; // Pivo central para o Ideal
const MIN_STOCK_USER = 3;
const WINDOW_DAYS = 730; 

function processNetwork() {
    console.log("🧬 Iniciando Experimento Apollo v15.5 (Bandas de Sigma)...");
    ...
    for (let storeName in rawData) {
        ...
        for (let t = WINDOW_DAYS; t < fullTimeline.length; t++) {
            ...
            const windowWithImputation = window.map(w => {
                ...
                const isRupture = available < (rawAvg * 0.5); 
                if (!isRupture) {
            ...
            finalTimeline.push({
                ...
                // Status baseados em Sigma a partir do Ideal (Ideal = ACIMA)
                p10: idealValue - 3 * sigmaTotal,  // RUPTURA
                p50: idealValue - 2 * sigmaTotal,  // CRÍTICO
                p100: idealValue - 1 * sigmaTotal, // ABAIXO
                p150: idealValue,                  // ACIMA (IDEAL)
                p200: idealValue + 1 * sigmaTotal, // MUITO ACIMA
                p400: idealValue + 2 * sigmaTotal, // ENCALHADO 1
                p800: idealValue + 3 * sigmaTotal  // ENCALHADO 2
            });
        }
    ...
    console.log("💎 Apollo v15.5 SIGMA EXPERIMENT Processed Successfully.");
}
