...
/**
 * APOLLO ENGINE v16.6 - FINAL HYBRID P100
 * Inteligência: Adaptive Window (730d) + Fourier + Slope
 * Escalas: Centradas no P100 (Eficiência Máxima - Sem dupla proteção)
 * Visualização: Hybrid (Inteligência adaptativa + Categorias fixas)
 */
...
function processNetwork() {
    console.log("🧬 Iniciando Motor Apollo v16.6 FINAL (Pivô 100% de Eficiência)...");
    ...
    for (let storeName in rawData) {
        ...
        for (let t = WINDOW_DAYS; t < fullTimeline.length; t++) {
            ...
            for (let i = 0; i < WINDOW_DAYS; i++) { sumX += i; sumY += imputedWindowSales[i]; sumXY += i * imputedWindowSales[i]; sumXX += i * i; }
            ...
            finalTimelineRaw.push({
                date: fullTimeline[t].date,
                physicalStock: fullTimeline[t].stock,
                availableStock: fullTimeline[t].stock - VITRINE_LOCAL,
                sales: fullTimeline[t].sales,
                rawDemand: demand,
                rawSigma: sigmaTotal
            });
        }
        ...
        for (let i = 0; i < finalTimelineRaw.length; i++) {
            ...
            const avgSigma = slice.reduce((a, b) => a + b.rawSigma, 0) / slice.length;
            
            // --- CALIBRACAO v16.6 (PIVO 100%) ---
            const targetBase = (avgDemand * LEAD_TIME_LOCAL) + (Z_SCORE_TARGET * avgSigma);
            ...
            finalTimelineFluid.push({
                ...
                p10:  targetBase * 0.1,  // RUPTURA
                p50:  targetBase * 0.5,  // CRÍTICO
                p80:  targetBase * 0.8,  // ABAIXO
                p100: targetBase * 1.0,  // ACIMA (IDEAL)
                p150: targetBase * 1.5,  // MUITO ACIMA
                p300: targetBase * 3.0,  // ENCALHADO 1
                p600: targetBase * 6.0   // ENCALHADO 2
            });
        }
    ...
    console.log("💎 Apollo v16.6 HYBRID P100 Processed Successfully.");
}
