...
/**
 * APOLLO ENGINE v17.1 - REPLENISH CALIBRATION
 * Inteligência: Lead Time Real (MA90) + No-Buffer.
 * STATUS: Foco em 100% (Qualquer coisa < 100% é ABAIXO).
 * REPOSIÇÃO: Alvo em 150% (Top-up para garantir cobertura operacional).
 */
...
function processNetwork() {
    console.log("🧬 Iniciando Motor Apollo v17.1 (Correção de Status e Reposição 150%)...");
    ...
    for (let storeName in rawData) {
        ...
        const focusIdeal = Math.max(Math.ceil(lastDay.p100), MIN_STOCK_USER);
        const topupTarget = Math.max(Math.ceil(lastDay.p150), MIN_STOCK_USER);
        
        let status;
        const avail = lastDay.availableStock;
        
        // --- NOVO CÁLCULO DE STATUS v17.1 ---
        if      (avail <  lastDay.p10)  status = "RUPTURA";
        else if (avail <  lastDay.p50)  status = "CRÍTICO";
        else if (avail <  lastDay.p100) status = "ABAIXO"; // Aqui captura o erro dos 84.7%
        else if (avail <  lastDay.p150) status = "ACIMA";
        else if (avail <  lastDay.p300) status = "MUITO ACIMA"; // Escalado para manter hierarquia
        ...
        networkResults[storeName] = {
            metrics: {
                ...
                estoqueIdeal: focusIdeal,
                repor: Math.max(0, topupTarget - avail), // Reposição agora mira o P150
    ...
    console.log("💎 Apollo v17.1 REPLENISH-P150 Processed Successfully.");
}
