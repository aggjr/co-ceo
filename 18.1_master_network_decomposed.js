...
/**
 * APOLLO ENGINE v18.1 - DECOMPOSED NETWORK HUB
 * Especial CD: Expõe o cálculo 'Próprio + Rede = Total' no card de reposição.
 */
...
function processNetwork() {
    console.log("🧬 Iniciando Motor Apollo v18.1 (Decomposição Próprio + Rede)...");
    ...
    console.log("💎 Apollo v18.1 DECOMPOSED HUB Processed Successfully.");
}
...
function processUnit(storeName, moves, otherStoresData = null) {
    ...
    // --- AGREGACÃO v18.1 (PEGA DADOS DE ONTEM DAS LOJAS) ---
    if (storeName === 'Fábrica' && otherStoresData && idxFinal > 0) {
        for (let s in otherStoresData) {
            // Usamos idxFinal - 1 (Ontem) para o pulso de rede
    ...
    const replenishTargetTotal = p150_proprio + pulsoRede;
    let healthFactor = Math.max(Math.ceil(lastDay.p100), MIN_STOCK_USER);
    ...
    return {
        metrics: {
            vitrine: VITRINE_LOCAL, currentStatus: status, stockHealth: (avail / lastDay.p100) * 100, currentAvailable: avail, currentPhysical: lastDay.physicalStock, minStock: MIN_STOCK_USER,
            estoqueReposicaoProprio: p150_proprio, // O "X"
            pulsoRede: pulsoRede,                   // O "N"
            estoqueReposicao: replenishTargetTotal, // O "T"
            estoqueSugestao: replenishTargetTotal - avail,
