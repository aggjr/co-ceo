...
/**
 * APOLLO ENGINE v16.0 - FLUID ADAPTIVE SIGMA
 * Centro (ACIMA) = 1.0 * [ (Demand * LT) + (Z * SigmaTotal) ]
 * Filtro de Suavização (Moving Average 30 dias) para eliminar aspecto 'pontiagudo'.
 */
...
const SMOOTH_WINDOW = 30; // Média móvel de 30 dias para fluidificação

function processNetwork() {
    console.log("🧬 Iniciando Motor Apollo v16.0 FLUIDO (Suavização 30d)...");
    ...
    for (let storeName in rawData) {
        ...
        // --- PASS 1: Geração de Dados Adaptativos Brutos ---
        ...
        // --- PASS 2: Fluidificação (Moving Average 30d) ---
        ...
        for (let i = 0; i < finalTimelineRaw.length; i++) {
            ...
            const idealValue = (avgDemand * LEAD_TIME_LOCAL) + (Z_SCORE_TARGET * avgSigma);
            ...
            finalTimelineFluid.push({
                ...
                p10:  idealValue - 3 * avgSigma, 
                p50:  idealValue - 2 * avgSigma, 
                p100: idealValue - 1 * avgSigma, 
                p150: idealValue,                  
                p200: idealValue + 1 * avgSigma, 
                p400: idealValue + 2 * avgSigma, 
                p800: idealValue + 3 * avgSigma  
            });
        }
    ...
    console.log("💎 Apollo v16.0 FLUID Network Processed Successfully.");
}
