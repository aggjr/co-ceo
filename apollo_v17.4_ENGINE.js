...
/**
 * APOLLO ENGINE v17.4 - REPLENISHMENT PULSE
 * STATUS: Calculado contra P100 (Eficiência).
 * REPOSIÇÃO: Alvo em 150% (P150).
 * SUGESTÃO: MAX(Reposicao, Minimo) - Disponível.
 * - Positivo: Precisa receber (Falta).
 * - Negativo: Pode doar (Excesso).
 */
...
function processNetwork() {
    console.log("🧬 Iniciando Motor Apollo v17.4 (Pulsos de Reposição Invertidos)...");
    ...
    for (let storeName in rawData) {
        ...
        // --- AUDITORIA DE RUPTURA E VENDAS EFETIVAS (PRÉ-PROCESSAMENTO) ---
        let ruptureCounterGlobal = 0;
        fullTimeline.forEach(day => {
            const available = day.stock - VITRINE_LOCAL;
            const isRupture = available <= 0;
            
            // Reset do orçamento de injeção apenas em vendas reais (ignora ruídos de ajuste)
            if (day.sales > 0) ruptureCounterGlobal = 0;
            else if (isRupture) ruptureCounterGlobal++;

            day.ruptureDur = ruptureCounterGlobal;
            day.isRupture = isRupture;
        });
        ...
        for (let t = WINDOW_DEMAND; t < fullTimeline.length; t++) {
            ...
            windowDem.forEach(w => {
                if (!w.isRupture) { healthySalesSum += w.sales; healthyDaysCount++; }
            });
            const healthyAverage = healthyDaysCount > 0 ? (healthySalesSum / healthyDaysCount) : rawAvg;

            // Injeção de vendas limitada a 15 dias (Regra de Decaimento Natural por Dados)
            const imputedWindowSales = windowDem.map(w => {
                if (w.isRupture && w.ruptureDur <= 15) return Math.max(healthyAverage, w.sales);
                return w.sales; // Rumo normal a partir do 16o dia ou fora de ruptura
            });
            ...
            const demand = Math.max(0, fourier + slope * WINDOW_DEMAND); // Removido limitador 0.01; protege apenas contra negativos.
        ...
        for (let i = 0; i < finalTimelineRaw.length; i++) {
            ...
            finalTimelineFluid.push({
                ...
                // Invariante: espelho do legado = físico auditado (unidades inteiras). Divergência = erro de dados.
        ...
        // --- NOVA FÓRMULA v17.4 (INVERTIDA) ---
        // Sugestão = MAX(Reposição, Mínimo) - Disponível
        // + : Receber (Falta)
        // - : Doar (Excesso)
    ...
    console.log("💎 Apollo v17.4 PULSE FORMULA Processed Successfully.");
}
